...
TOKEN_FROM_URL = os.environ.get('TOKEN_FROM_URL')
...
def update_expired_access_token():
    ...
    # Save json response as a variable
    updated_strava_tokens = response.json()
    # Save tokens to file
    with open(strava_tokens_outfile, 'w') as outfile:
        json.dump(updated_strava_tokens, outfile)
    # Print the tokens to screen to confirm authentication was successful
    print(updated_strava_tokens)
...
def get_activities_and_create_csv():
    ...
    while True:
        full_activities_data = requests.get(url + '?access_token=' + access_token + '&per_page=10' + '&page=' + str(page))
